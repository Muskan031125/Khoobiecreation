...
<title><?= esc($page['title'] ?? 'Partner Login') ?></title>
...
<div class="w-full max-w-sm">
    <div class="bg-white rounded-2xl shadow-xl p-7">
        <div class="text-center">
            ...
            <h1 class="mt-3 text-xl font-black">Partner Portal</h1>
            <p class="text-xs text-slate-500">Sign in to manage your orders</p>
        </div>
        ...
        <form method="post" action="<?= base_url('partner/login') ?>" class="mt-6 space-y-3">
            <?= csrf_field() ?>
            <div style="position:absolute;left:-9999px;width:1px;height:1px;overflow:hidden;" aria-hidden="true">
                <input type="text" name="website" tabindex="-1" autocomplete="off" value="">
            </div>
            <input name="identifier" required placeholder="Email or phone" value="<?= esc(old('identifier')) ?>" class="w-full px-4 py-3 rounded-lg border border-slate-200">
